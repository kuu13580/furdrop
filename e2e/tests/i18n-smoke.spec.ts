 ...
 *
 * TRANSLATED は翻訳が済んだページを列挙する。画面を追加したらここにも足す。
 * 直接 goto できる画面に限る (送信中 / 送信完了は直接開くと前の画面へ
 * リダイレクトされるため、送信フロー spec 側でカバーする)。
 *
...
test("使い方ガイド: en では英語の図版を参照する (目的: 撮り漏れ・撮り忘れの検出)", async ({
  ...
  for (const tab of ["Sender", "Receiver"]) {
