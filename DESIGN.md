...
| **Amber Tint** | `#FBF0DE` | 警告バナー背景 (Alert `warn`) / 期限が近い削除予定日の下地。Amber は明度が高く、文字色・白文字とも AA を満たさないため、文字は必ず Ink にする |
...
- 残り 14 日以内: 背景 Amber `#D98F2E` / 文字 **Ink** (白文字では 2.65:1 で AA を満たさない)
- 残り 3 日以内: 背景 Rust `#A8381F` / 文字 白 (6.4:1)
...
- **詳細画面 (S08)**: 拡大表示。**アスペクト比を必ず保持する**。コンテナは `aspect-ratio: width / height` を持たせたうえで、`width: min(100%, calc(70dvh * ratio))` で幅を決める。こうすると縦辺は必ず `70dvh` 以下に収まり、横辺は親幅を超えず、どの画面サイズ・縦横比の写真でも歪みなく表示できる。`vh` ではなく `dvh` を使うのは iOS Safari の URL バー出し入れに追従させるため (`vh` は最大ビューポート基準で URL バー表示時に画像がはみ出る)。固定 `height` + `aspect-ratio` + `max-width` の組み合わせは**禁止** (モバイル幅に clamp された瞬間にアスペクト比が崩れ、サムネイルが歪むため)。`object-fit: contain` を画像自体にも適用。ロード中サムネイルのブラーも `object-contain` を使い、コンテナのアスペクト比に頼ってクロップしない。
...
- **詳細画像で固定 `height` + `aspect-ratio` + `max-width` を併用しない** — モバイル幅に clamp されるとアスペクト比が崩れる。必ず `width: min(100%, calc(70dvh * ratio))` で幅を決めて縦辺を間接制御する (`vh` は iOS で URL バー出し入れに追従しないため不可)
