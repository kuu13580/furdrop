...
| S13 | メールアドレスの確認 | `/verify-email?token=` | 不要 | 確認メールのリンク先 (R09)。`POST /notifications/verify-email` を叩いて結果を出す。**ログインしていない前提** — メールアプリから踏まれる |
| S14 | 通知の配信停止 | `/unsubscribe?t=&k=` | 不要 | メールフッターの解除リンク先 (R09)。**確認ボタンを押させてから**解除する (メールセキュリティスキャナのリンク自動巡回で勝手に解除されるのを防ぐ) |
...
flowchart TD
    ...
    S10 -->|アカウント削除| S05
    S10 -.->|通知先を登録<br/>確認メール| S13[S13: メールアドレスの確認<br/>認証不要]
    S13 --> S10
    MAIL[通知メール] -.->|フッターの解除リンク| S14[S14: 通知の配信停止<br/>認証不要・確認ボタン]
```
...
- **アドレスは検証してから使う。** 保存すると確認メールが飛び、リンクを開くまで `notification_email` に入らない。検証待ちのあいだは入力欄の下に状態行 (「確認メールを送りました」+ 再送 / 取り消し) を出す
- **アドレス変更中も旧アドレスに届く。** 新しいアドレスの検証が済むまで切り替わらない
- **オン/オフは種類ごとに 3 つ。** ワンクリック解除が倒すスイッチを「うるささの単位」と一致させるため。1 つにまとめると「ダイジェストがうるさい」で解除した人が削除予告とクォータ警告も失う
- **入力欄と保存ボタンの間に利用目的を 1 行出す。** フォームでの直接取得は個情法21条2項の「明示」が要り、ポリシーへの掲載 (公表) では足りない
- 既定は 3 種とも ON。アドレスを検証した時点で「受け取る」意思表示とみなす
