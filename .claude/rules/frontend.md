...
- **`App.tsx` の `<Route path=...>` を触ったら `analytics.ts` の `KNOWN_PATH_SEGMENTS` も直す**
  (追加だけでなく、削除・改名のときも整理する)。許可リスト方式なので、足し忘れても値は漏れないが
  GA 上で `/*` に潰れて集計できなくなる。**lint も型もテストも通るので CI では検出できない**
- `/*` は 2 つの意味を兼ねている。`/send/*` は**既知ルートのパラメータをマスクした結果**、
  `/*` は**ルートごと未知** (許可リスト漏れ、または 404)。GA で `/*` にトラフィックが立っていたら
  足し忘れを疑う
- **既知の負債**: この許可リストは `App.tsx` のルート定義を手書きで複製したもので、二重管理に
  なっている。ルート定義を単一のソースにして**そこから導出すれば足し忘れは構造的に起きなくなる**
  (`:param` を除いた静的セグメントがそのまま許可リストになる)。そのとき未一致のパスも `/*` ではなく
  明示的な値 (`/unmatched` 等) に落とせて、上記の 2 つの意味を分離できる
